...
class TestShowApplication:
    ...
    def test_conference_not_found_returns_404(self, client, mock_services):
        mock_services["conference"].check_registration_open.side_effect = ConferenceNotFoundException(1)
        resp = client.get("/conference/1/application")
        assert resp.status_code == 404

    def test_conference_already_ended_returns_409(self, client, mock_services):
        mock_services["conference"].check_registration_open.side_effect = ApplicationAfterDeadlineException("2025-01-01")
        resp = client.get("/conference/1/application")
...
class TestShowUploadThesis:
    ...
    def test_conference_not_found_returns_404(self, client, mock_services):
        mock_services["conference"].check_submission_open.side_effect = ConferenceNotFoundException(1)
        resp = client.get("/conference/1/thesis")
        assert resp.status_code == 404

    def test_conference_already_ended_returns_409(self, client, mock_services):
        mock_services["conference"].check_submission_open.side_effect = ThesisAfterDeadlineException("2025-01-01")
        resp = client.get("/conference/1/thesis")
        assert resp.status_code == 409
