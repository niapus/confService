...
class ScheduleService:
    def __init__(self, conf_service: ConferenceService, thesis_service: ThesisService,
                 schedule_mapper: ScheduleMapper, schedule_repository: ScheduleRepository,
    ...
    def get_full_schedule_data(self, conf_id, session) -> FullScheduleDTO:
    ...
    def get_schedule(self, conf_id, session):
        return self.__repo.get_by_conference_id(conf_id, session)

    def update_schedule(self, schedule_dto: ScheduleDTO, conf_id, session):
        conference = self.__conf_service.get_conference_by_id(conf_id, session)
        deleted_count = self.__repo.delete_all_by_conference_id(conf_id, session)
        items = []

        for item in schedule_dto.schedule:
            items.append(self._fill_schedule_item(item, conf_id))
    ...
    def _fill_schedule_item(self, item_dto: ScheduleItemDTO, conf_id):
        return ScheduleItem(
